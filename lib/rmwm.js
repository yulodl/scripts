  ...
  (0, _child_process.exec)('mediainfo -f ' + mf + ' | grep  -E \'^Width.*\\d+$|^Height.*\\d+$\'', function (error, stdout, stderr) {
    ...
    (0, _child_process.exec)('ffmpeg -loglevel quiet -ss 1 -i ' + mf + ' -vframes 1 -q:v 2 ' + mf + '.jpg', { timeout: 5 * 1000 }, function (error, stdout, stderr) {
      ...
      _colorExtractor.imagemagick.getColor(mf + '.jpg', {
        ...
        (0, _child_process.exec)('ffmpeg -loglevel quiet -i ' + mf + ' -f lavfi -i color=c=0x' + overlayColor + ':s=' + sPX.size + ' -vcodec h264 -acodec copy -profile:v main -tune stillimage -filter_complex \'overlay=' + sPX.positionX + ':0:shortest=1\' ' + mfHandled, { timeout: 2 * 60 * 1000 }, function (error, stdout, stderr) {
          if (error) {
